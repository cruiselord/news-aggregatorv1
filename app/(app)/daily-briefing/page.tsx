...
export default async function DailyBriefingPage() {
  ...
  let heroStory: Story | null = null
  let gridStories: Story[] = []
  let alsoInNews: Story[] = []
  let blindspotStory: Story | null = null

  if (!briefingError && briefing && Array.isArray(briefing.story_ids)) {
    const { data: clusters } = await supabase
      .from("story_clusters")
      .select("*")
      .in("id", briefing.story_ids as string[])
    ...
    const hero =
      mapped.find((s) => s.id === briefing.headline_story_id) ?? mapped[0]

    heroStory = hero ?? null
    gridStories = mapped.filter((s) => s.id !== hero?.id).slice(0, 6)
    alsoInNews = mapped.slice(1, 6)
    blindspotStory =
      mapped.find((s) => s.id === briefing.blindspot_story_id) ??
      mapped.find((s) => s.isBlindspot) ??
      mapped[1] ??
      null
  }
    ...
    <div className="p-6">
      ...
      <div className="mb-8 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {briefing
            ? `${(briefing.story_ids as string[] | null)?.length ?? 0} stories · ${
                briefing.total_articles ?? 0
              } articles · ~${
                briefing.estimated_read_minutes ?? 12
              } min read · Curated by AI`
            : "Curated by AI from today’s top clusters"}
        </p>
        ...
        <div className="mb-8 rounded-lg border border-border bg-card p-6">
          <div className="flex gap-6">
            ...
            <div className="flex flex-1 flex-col justify-between">
              <div>
                <Badge className="mb-2 bg-[#008751] text-[#ffffff] text-xs">
                  TOP STORY
                </Badge>
          ...
          <ul className="flex flex-col gap-2">
            {alsoInNews.map((story) => (
              <li key={story.id}>
                <Link
                  href={`/story/${story.id}`}
                  className="flex items-center gap-2 text-sm text-foreground hover:text-[#008751]"
                >
          ...
          <div className="rounded-lg border-2 border-[#FF6D00] bg-[#FF6D00]/5 p-6 transition-shadow hover:shadow-md">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                {blindspotStory.blindspotPercent && blindspotStory.blindspotSide && (
                  <p className="mt-1 text-sm text-muted-foreground">
                    {blindspotStory.blindspotPercent}% of coverage comes from{" "}
                    {blindspotStory.blindspotSide} sources. See what the other
                    side is saying.
                  </p>
                )}
